...
export const DEFAULTS = {
  ...
  // boundary ring alone.
  taperRings: 1,
  // Rings of the boundary forced fully faded ON TOP of whatever the fade shape
  // is doing, so a fade panel can also present a plain edge. 0 is off.
  taperBorder: 0,
  taperSharp: 100,
...
export function quantileRank(values) {
  ...
  // AND THE TOP GROUP MUST REACH 1.
  //
  // Ranking a tie group by its first index over n cannot: the last group starts
  // at n minus its own size, so it lands short by exactly that group's share of
  // the field. With contrast at 100 the largest holes therefore never reached
  // max dia - 43 of the saved designs were affected, including the standard
  // ones, so Basic-50-1225 asked for 25mm and cut 24.55. Dividing by where the
  // LAST group starts puts the smallest value at 0 and the largest at 1 while
  // ties still share a rank, which is the property the note above is about.
  let lastStart = 0;
  for (let a = 0; a < n; ) {
    let b = a;
    while (b + 1 < n && values[idx[b + 1]] === values[idx[a]]) b++;
    lastStart = a;
    a = b + 1;
  }
  const denom = lastStart > 0 ? lastStart : 1;
  let i = 0;
  while (i < n) {
    let j = i;
    while (j + 1 < n && values[idx[j + 1]] === values[idx[i]]) j++;
    const r = n <= 1 ? 0 : Math.min(1, i / denom);
...
function taperFieldAt(x0, y0, p, f) {
  ...
  switch (p.taperDriver ?? 'linear') {
    // The fade counted in rows, so a fade panel arrives at EXACTLY the small
    // hole size and meets the plain field it butts against. Every other shape
...
function taperAt(x, y, p, f) {
  ...
  // A BORDER ON TOP OF WHATEVER THE FADE IS DOING.
  //
  // Once the field it butts against carries a 12.5 ring, a fade panel needs one
  // too - its INNER edge meets that ring, and a fade is 0 there, which is full
  // pattern. Taking the larger of the two gives the panel both: the boundary
  // ring lands on the small size on all four sides, and the fade runs normally
  // everywhere inside it.
  const bRings = Math.max(0, Math.round(p.taperBorder ?? 0));
  if (bRings > 0) {
    // Measured on the same frame the fade is, so the ring lands on the panel
    // edge rather than somewhere near it.
    const bf = taperFrame(x, y, p, f);
    const bi = latticeIndex(bf.x, bf.y, bf.w, bf.h, p);
    if (bi) {
      const d = Math.min(bi.i, bi.nx - bi.i, bi.j, bi.ny - bi.j);
      u = Math.max(u, clamp(1 - d / bRings, 0, 1));
    }
  }
  const s = amount * u;
...
/**
 * A ramp counted in the lattice's OWN rows and columns, in [0,1].
 *
 * Shared by the size driver and the fade layer, because both need the same
 * property and for the same reason: the first line has to be exactly one end
 * and the last exactly the other, or the panel will not meet the one it butts
...
function rampCount(x, y, nw, nh, a, p) {
  ...
  //
  // A staggered lattice offsets every other row by half a pitch, so along that
  // axis the holes sit half a pitch apart. Counting whole cells rounded each
  ...
  //
  // Along the top edge the down term is 0, so the value IS the across ramp;
  // along the left edge the across term is 0, so it IS the down ramp. The
  // contours turn the corner in an L and the panel hands off to both.
  // The angle picks the corner: 45 top-left, 135 top-right, 225 bottom-right,
  // 315 bottom-left.
  if (p.rampCorner) return Math.max(u, v);
  // Otherwise a ramp follows ONE axis; angles between snap to the nearer.
  // A diagonal was tried and removed: identical to 'linear' at every angle
  // whenever the span is a whole number of rows, and its four edges all vary
  // so it matched no edge of any panel in the set.
  return Math.abs(c) >= Math.abs(s) ? u : v;
}


/**
 * The 35 diamond cells of one weave unit, as (i, j) offsets from its centre.
 *
 * Read out of the reference drawing rather than composed: ref.dxf's unit
 * outline was tested against the rhombic lattice it is drawn on, and these are
 * the cells whose centres fell inside. Rows run j = 7 at the top tip down to
 * j = -7 at the bottom, three cells wide through the middle and one at each end.
 */
let WEAVE_MOTIF = null;
export function weaveMotif() {
  if (WEAVE_MOTIF) return WEAVE_MOTIF;
  // Thirteen rows, two cells wide, tapering to one at each tip: down-left for
  // three rows, turn, down-right for seven, turn, down-left for three. That is
  // the Z in the drawing, and these 24 cells tile the plane exactly at a pitch
  // of 6 across and 8 down with every second column dropped by 4 - checked, no
  ...
  WEAVE_MOTIF = new Set(cells.map((c) => c[0] + ',' + c[1]));
  return WEAVE_MOTIF;
}
...
function modulate(x, y, p, f) {
  ...
  switch (p.modulation) {
    ...
    case 'herringbone': {
      // THE UNIT, TAKEN OFF THE DRAWING CELL BY CELL.
      //
      // A cell is a DIAMOND and a unit is a chain of 24 of them - see
      // weaveMotif. The size is how far a cell sits from the middle of its own
      // unit, so a unit is largest at its two tips and smallest at its waist,
      // which is the gradient the drawing shades.
      //
      // The numbers are the drawing's: ref.dxf was parsed, its unit outlines
      // tested against the lattice, and the result checked by redrawing it. The
      // one thing that had to be got right to read it at all is the PARITY -
      // the polygon's corners sit on the same lattice as its cells' centres
      // would on the obvious choice, so a boundary ran straight through the
      // points being tested and every marginal cell came out a coin toss. The
      // cells are on the other parity, and with that the two shapes come out as
      // exact mirrors of one another, 24 cells each.
      const per = driverPeriod(p);
      const pairs = Math.max(1, Math.round(per.w / (2 * Math.max(1, p.weaveW ?? 150))));
      const uw = per.w / (2 * pairs); // one unit column
      const rows = Math.max(1, Math.round(per.h / Math.max(1, p.weaveL ?? 300)));
      const uh = per.h / rows; // one unit down the column
      const a = uw / 6;
      const b = uh / 8;
      // WHICH CELL A HOLE IS IN, DECIDED BY THE DIAMOND AND NOT BY ROUNDING.
      //
      // The cells are diamonds on the odd parity of an a-by-b grid, and those
      // diamonds tile the plane exactly - each has area 2ab and there is one
      // per 2ab of lattice. So every point belongs to exactly one, and finding
      // it is a question of which centre is nearest in the DIAMOND's own
      // metric, |dx| / a + |dy| / b.
      //
      // Rounding x and y separately and then nudging the parity is not that.
      // It rounds into a RECTANGLE and then, whenever the parity came out
      // wrong, shifts right - always right, never left and never vertically -
      // so a band down the left of every cell was being handed to its
      // neighbour. That is why cell populations came out 1, 2, 3 and 4 holes
      // when the geometry says they should all be the same.
      //
      // Rotating into s = u + v, t = u - v turns the L1 ball into an L-infinity
      // one: the diamond becomes a unit square, and the nearest centre is just
      // the nearest odd integer on each axis independently. No ties, no nudge.
      const u = x / a;
      const v = y / b;
      const nearestOdd = (z) => 2 * Math.round((z - 1) / 2) + 1;
      const sn = nearestOdd(u + v);
      const tn = nearestOdd(u - v);
      const ci = (sn + tn) / 2;
      const cj = (sn - tn) / 2;
      const M = weaveMotif();
      // Which unit owns this cell. The tiling is exact, so exactly one will.
      for (let dc = -1; dc <= 1; dc++) {
        for (let dr = -1; dr <= 1; dr++) {
          const c = Math.round(ci / 6) + dc;
          const odd = (((c % 2) + 2) % 2) === 1;
          const r = Math.round((cj - (odd ? 4 : 0)) / 8) + dr;
          const di = ci - 6 * c;
          const dj = cj - 8 * r - (odd ? 4 : 0);
          if (!M.has((odd ? -di : di) + ',' + dj)) continue;
          // THE GRADIENT RUNS THROUGH THE CELL, NOT IN STEPS OF IT.
          //
          // The cell decides which UNIT a hole belongs to. It should not also
          // decide the hole's size: returning |dj| / 6 off the cell's integer
          // row makes every hole in a cell exactly the same size, so a cell is
          // a flat block and the ramp only changes where one cell ends and the
          // next begins. That is the stepping between neighbouring blocks.
          //
          // Once the unit is known, the distance from its middle is a real
          // number - v is y in cell-rows already - so the ramp is continuous
...
export function buildField(params) {
  ...
  if (acts.size && candidates.length) {
    ...
    // WHERE THE FADE BOTTOMS OUT.
    //
    // A design that states a size RANGE has already said what its small end
    // is, and that is where the fade has to land: asking for 12.5 and getting
    // 12 leaves a fade panel that cannot meet the 12.5 field beside it. Only a
    // uniform design has no stated small end - there minR IS the hole size, so
    // fading toward it would do nothing, and the product floor is the sensible
    // target instead.
    const floorR =
      p.minDia < p.maxDia ? minR : Math.min(minR, LIMITS.practicalFloor / 2);
    for (const c of candidates) {
  ...
  if (barMax > 0) {
    ...
    // AND THE ROWS HAVE TO REPEAT WITH THE PANEL TOO.
    //
    // The columns were made panel-periodic and the rows were not, which is a
    // half-fix that looks like a whole one. Rows are counted from the field
    // origin, so unless the block size happens to divide the rows in a panel -
    // 9 into 34, it does not - panel row 0 and panel row 1 break at different
    // heights, and the two present different bars on the seam they share. That
    // is a mismatch under P1 as much as under P4; it was missed because the
    // check being run compared two panels of the same row.
    //
    // Counting the row within the panel fixes it. The block that meets the
    // panel edge is then a short one rather than a full K, which only shortens
    // some bars - it never lengthens one past the limit.
    const rowsPerPanel = Math.max(1, Math.round(PANEL.moduleH / py));
    const rowIn = (c) => (((rowOf(c) % rowsPerPanel) + rowsPerPanel) % rowsPerPanel);
    const isBreak = (c) => ((((rowIn(c) - phaseOf(c)) % K) + K) % K) === 0;
  ...
  // WHAT WAS ACTUALLY CUT, AGAINST WHAT WAS ASKED FOR.
  //
  // The size a design states and the size it delivers had drifted apart in
  // several independent ways at once - the pitch capping it, a ramp spanning
  // further than the panel, a rank that never reached the top - and none of
  // them said anything. Reporting the delivered range catches all of them and
  // any future one: if a design says 25mm and cuts 24.5, that is now on screen
  // rather than discovered in a DXF.
  if (holes.length) {
    let lo = Infinity;
    let hi = -Infinity;
    ...
    stats.diaLow = +lo.toFixed(3);
    stats.diaHigh = +hi.toFixed(3);
    // Judged against what the lattice could carry, not the raw request: a
    // diameter the pitch refuses is already reported separately as diaClamped.
    const wantHigh = Math.min(p.maxDia, diaCap);
    const wantLow = Math.max(Math.min(p.minDia, p.maxDia), LIMITS.minDia);
    stats.diaShort = hi < wantHigh - 0.01 || lo > wantLow + 0.01;
    stats.diaWant = [+wantLow.toFixed(3), +wantHigh.toFixed(3)];
  }
