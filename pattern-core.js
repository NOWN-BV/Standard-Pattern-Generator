...
function modulate(x, y, p, f) {
  ...
  switch (p.modulation) {
    ...
    case 'herringbone': {
      ...
      const a = uw / 6;
      const b = uh / 8;
      let ci = Math.round(x / a);
      const cj = Math.round(y / b);
      // Cells live on the odd parity; relabel rather than reject, so a lattice
      // that happens to sit on the other one still reads.
      if ((((ci + cj) % 2) + 2) % 2 === 0) ci += 1;
      const M = weaveMotif();
