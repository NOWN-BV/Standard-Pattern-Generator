...
function modulate(x, y, p, f) {
  ...
  switch (p.modulation) {
    ...
    case 'herringbone': {
      // MEASURED OFF THE DRAWING, not guessed at. The reference tiles as
      // interlocking zigzag ribbons: columns at one pitch, units stacked at
      // another down each column, and every second column dropped by HALF a
      // unit so the two interlock. Its own numbers are 519.4 and 1093.6mm,
      // a ratio of 1 to 2.105, and the two shapes in it are mirror images.
      //
      // The size runs large at the two TIPS of a unit and small at its waist,
      // so the field is simply how far you are from the middle of the unit you
      // are in. Where two units meet, both are at their tip - that band of
      // large holes is the seam of the weave, and the small waists are what
      // separate one stroke from the next.
      const per = driverPeriod(p);
      ...
      // The arms of a unit slope, so the bands do too - lean is the drawing s
      // own 519.4 across per 820.2 down, mirrored in the odd columns.
      const lx = x - col * w;
      const lean = clamp(p.weaveLean ?? 63, 0, 200) / 100;
      const yy = y + (odd ? Lr / 2 : 0) + (odd ? -1 : 1) * lean * (lx - w / 2);
      const ly = yy - Math.round(yy / Lr) * Lr;
      return Math.min(1, Math.abs(ly) / (Lr / 2));
    }
