...
const SPEC = [
  ...
  },
  {
    ...
    options: () => ['seam', 'centre'],
    labels: (v) => (v === 'centre' ? 'the centre' : 'the seam'),
    when: (st) => st.modulation === 'wedge',
  },
  {
    key: 'crossDuty',
...
const TIPS = {
  ...
  modAngle: 'Direction the ramp runs: 0 left to right, 90 top to bottom, 180 and 270 the other way. A ramp follows an AXIS - angles between snap to the nearer one, because a diagonal ramp matched no edge of any panel in the set and was identical to LINEAR anyway. Use linear for a gradient at an arbitrary angle. With TURN THE CORNER on, 45 / 135 / 225 / 315 pick which corner holds the fine end.',
  rampCorner:
